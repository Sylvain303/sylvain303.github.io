...
async function applyActions(actions) {
  ...
  try {
    ...
    const addIndex = actions.findIndex(a => a[0] === 'AddRecord' || a[0] === 'BulkAddRecord');
    if (addIndex !== -1) {
      const ret = retValues[addIndex];
      // AddRecord returns a single id; BulkAddRecord returns an array of ids
      const newRowId = Array.isArray(ret) ? ret[0] : ret;
      if (newRowId) {
